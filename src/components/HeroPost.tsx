...
export default function HeroPost({ imagemUrl, categoria, titulo, resumo, slug }: HeroPostProps) {
  return (
    <div className="relative bg-white border border-neutral-100 rounded-lg overflow-hidden shadow-lg group">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 text-white font-body">
        {categoria && (
          <Link href={`/categoria/${categoria.slug}`} className="font-heading text-sm font-bold uppercase bg-primary-dark px-3 py-1 rounded-full inline-block mb-3 hover:bg-primary transition-colors">
            {categoria.nome}
          </Link>
        )}
        ...
        <p className="hidden md:block text-neutral-100 mt-2 text-lg leading-relaxed">
          {resumo}
        </p>
      </div>
    </div>
